...
class zabbix_api:
    def __init__(self):
        self.url = 'http://172.17.73.69/api_jsonrpc.php'
    ...
    def user_login(self):
        ...
        try:
            ...
            print "\033[041m auth fail,please check URL !\033[0m",e.code
        except KeyError as e:
            print "\033[041m auth fail ,please check user and password !\033[0m",e
        else:
    ...
    def hostid_get_hostname(self, hostId=''):
        ...
        try:
            ...
            #print "host number: \33[31m%s\33[0m" % (len(response['result']))
            host_dict=dict()
    ...
    def host_get(self,hostName=''):
        ...
        try:
            ...
            print "host number: \033[31m%s\033[0m"%(len(response['result']))
            for host in response['result']:
                status={"0":"OK","1":"Disabled"}
                available={"0":"Unknown","1":"available","2":"Unavailable"}
                #print host
    ...
    def host_create(self, hostIp, hostgroupName, templateName, hostName):
        ...
        try:
            ...
            print "\033[041m add host has err!\033[0m",e
            print response
    ...
    def host_delete(self,hostNames):
        ...
        for hostName in hostNames.split(','):
            ...
            if not hostid:
                print "host delete \033[041m %s\033[0m fail!" % hostName
                sys.exit()
        ...
        try:
            result = urllib2.urlopen(request)
            result.close()
            print "host \033[041m %s\033[0m delete !" % hostName
        except Exception,e:
            print  e
...
if __name__ == "__main__":
    ...
    parser.add_argument('-H','--host',nargs='?',dest='listhost',default='host',help='host query')
    parser.add_argument('-G','--group',nargs='?',dest='listgroup',default='group',help='group query')
    parser.add_argument('-T','--template',nargs='?',dest='listtemp',default='template',help='template query')
    parser.add_argument('-A','--add-group',nargs=1,dest='addgroup',help='add group')
    parser.add_argument('-C','--add-host',dest='addhost',nargs=4,metavar=('192.168.2.1', 'groupname', 'Template01,Template02', 'hostName'),help='add host')
    parser.add_argument('-d','--disable',dest='disablehost',nargs='+',metavar=('sh-aa-01'),help='disable host ')
    parser.add_argument('-e','--enable',dest='enablehost',nargs=1,metavar=('sh-aa-01'),help='enable host')
    parser.add_argument('-D','--delete',dest='deletehost',nargs='+',metavar=('sh-aa-01'),help='delte host')
