...
int interval[] = {0,1,2,4,6,8,10,15,20,25,30,40,50,60,70,80,90,100,120,150,200,300,400,500,1000,2000,4000,8000,16000,32000};
...
int buffering = 0 ;
int debug = 0 ;
unsigned long long endure_delay = 0;
...
int print_summary(struct statistic* summary)
{
	...
	fprintf(stdout,"max_latency          : %llu (us)\n", summary->max_latency);
...
static void print_result_and_exit()
{
	...
	for(idx = 0 ; idx < thread_num; idx++)
	{
		...
		if(args_array[idx].stat->max_latency > summary.max_latency)
		{
			summary.max_latency = args_array[idx].stat->max_latency;
		}
	}
...
int r_write(int fd, char* buffer, ssize_t size)
{
	...
	while(reserve_bytes > 0)
	{
		...
		{
			fprintf(stderr, "error happened when write, error No: %d\n", errno);
			return -1;
		}
	}
...
void print_timestamp(char* filename,ssize_t offset, ssize_t length,  unsigned long long tm_begin, unsigned long long tm_end, unsigned long long delay)
{
	char b_buffer[26] ; 
	char e_buffer[26] ; 

	time_t b_tv_sec = tm_begin /1000000 ; 
	int  b_tv_msec = (tm_begin % 1000000)/1000 ;
	time_t e_tv_sec = tm_end /1000000 ; 
	int  e_tv_msec = (tm_end % 1000000)/1000 ;

	struct tm b_tm_info ; 
	localtime_r(&b_tv_sec, &b_tm_info);
	struct tm e_tm_info ; 
	localtime_r(&e_tv_sec, &e_tm_info);

	strftime(b_buffer,26, "%Y-%m-%d %H:%M:%S", &b_tm_info);
	strftime(e_buffer,26, "%Y-%m-%d %H:%M:%S", &e_tm_info);
	if (delay == 0)
		fprintf(stdout, "%s.%03d ~ %s.%03d FINISH ONE FRAME \n", b_buffer,b_tv_msec, e_buffer, e_tv_msec);
	else
		fprintf(stdout, "%s.%03d ~ %s.%03d FINISH ONE FRAME (%-15s %10zd ~ %10zd) cost %7lld (ms)\n", 
				b_buffer, b_tv_msec, e_buffer, e_tv_msec, filename, offset, length, delay/1000);
...
void * worker(void* param)
{
	...
	for(i = 0; i < filenum ; i++)
	{
		...
		if (buffering == 0)
		{
			fd = open(filename, O_CREAT|O_RDWR|O_DIRECT, S_IRWXU|S_IRGRP|S_IROTH);
			if(fd < 0)
			{
		...
		while(length < filesize)
		{
			...
			{
				fprintf(stderr,"failed to write %s (%zd)\n",filename, length);
				pthread_exit(NULL);
			}
			else
			{
			...
			if(debug == 1)
			{
				print_timestamp(filename, length, current_size, tm_begin, tm_end, current_us);
			}
			else if(endure_delay > 0 && current_us > endure_delay*1000)
			{
				print_timestamp(filename, length, current_size, tm_begin, tm_end, current_us);
			}

			if(current_us > stat->max_latency)
			{
				stat->max_latency = current_us ;
			}
...
void usage()
{
	...
	fprintf(stdout, "-l   --delay      if delay (ms) big than this value, print timestamp and delay\n");
	fprintf(stdout, "-b   --prefix     file name's prefix \n");
	fprintf(stdout, "-B   --buffering  use fwrite instead of write \n");
	fprintf(stdout, "-D   --debug      print timestamp when finish one frame \n");
}
...
int main(int argc, char* argv[])
{
	...
	int c;
	const char* short_options = "h?d:p:s:f:S:n:b:l:BD";
